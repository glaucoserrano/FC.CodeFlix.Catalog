...
public class ListCategoriesTest
{
    ...
    public async Task SearchReturnsListAndTotal()
    {
        ...
        var categoryRespository = new CategoryRepository(dbContext);
        var SearchInput = new useCases.ListCategoriesInput(1, 20);
        var useCase = new useCases.ListCategories(categoryRespository);
    ...
    public async Task SearchReturnsPaginated(int quantityCategoriesToGenerate, int page, int perPage, int expectedQuantityItems)
    {
        ...
        await dbContext.SaveChangesAsync(CancellationToken.None);
        var categoryRespository = new CategoryRepository(dbContext);
        var SearchInput = new useCases.ListCategoriesInput(page, perPage, "", "", Domain.SeedWork.SearchableRepository.SearchOrder.Asc);
        var useCase = new useCases.ListCategories(categoryRespository);
    ...
    public async Task SearchByText(string search, int page, int perPage, int expectedQuantityItemsReturned, int expectedQuantityTotalItems)
    {
        ...
        await dbContext.SaveChangesAsync(CancellationToken.None);
        var categoryRespository = new CategoryRepository(dbContext);
        var SearchInput = new useCases.ListCategoriesInput(page, perPage, search, "", Domain.SeedWork.SearchableRepository.SearchOrder.Asc);
        var useCase = new useCases.ListCategories(categoryRespository);
    ...
    public async Task SearchOrdered(string orderBy, string order)
    {
        ...
        var SearchInput = new useCases.ListCategoriesInput(1, 20, "", orderBy, searchOrder);
        var useCase = new useCases.ListCategories(categoryRespository);
